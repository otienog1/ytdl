...
ExecStart=${VENV_PATH}/bin/uvicorn app.main:app --host 0.0.0.0 --port ${PORT} --workers 2
Restart=always
RestartSec=5
...
Environment="PATH=${VENV_PATH}/bin:/usr/bin:/bin"
EnvironmentFile=${ENV_FILE}
ExecStart=${VENV_PATH}/bin/celery -A app.queue.celery_app worker --loglevel=info --concurrency=2
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
...
Environment="PATH=${VENV_PATH}/bin:/usr/bin:/bin"
EnvironmentFile=${ENV_FILE}
ExecStart=${VENV_PATH}/bin/celery -A app.queue.celery_app beat --loglevel=info
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
