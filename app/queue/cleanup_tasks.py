...
async def _cleanup_old_downloads_async():
    """Async cleanup processing with reference counting"""
    try:
        ...
        # Find all downloads older than cutoff date
        old_downloads = await db.downloads.find({
            'createdAt': {'$lt': cutoff_date},
            'status': 'completed',
            'downloadUrl': {'$exists': True, '$ne': None}
        }).to_list(length=None)

        logger.info(f"Found {len(old_downloads)} downloads older than {expiry_hours} hours")

        files_deleted = 0
        records_updated = 0
        files_kept_due_to_references = 0

        # Process each old download
        for download in old_downloads:
            ...
            if recent_downloads > 0:
                ...
                files_kept_due_to_references += 1
        ...
        logger.info(
            ...
            f"{files_kept_due_to_references} files kept (active references), "
            f"{records_updated} records updated"
        )

        return {
            'files_deleted': files_deleted,
            'files_kept': files_kept_due_to_references,
